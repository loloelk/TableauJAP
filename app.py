...
config = load_config()
st.set_page_config(
    page_title="Tableau de Bord Patients JAP",
    page_icon="🧠",
    layout="wide",
...
def check_login():
    ...
    st.title("🔒 Connexion - Tableau de Bord JAP") #
...
if check_login():
    ...
    if st.session_state.first_visit_after_login:
        st.info(f"""
        ### Bienvenue au Tableau de Bord JAP, {st.session_state.get('username', 'Utilisateur')}! ({st.session_state.get('role', '').upper()})
    ...
    # Define psychosis assessment scales
    st.session_state.setdefault('PANSS_POSITIVE_ITEMS', [f'panss_pos_{i}' for i in range(1, 8)])
    st.session_state.setdefault('PANSS_NEGATIVE_ITEMS', [f'panss_neg_{i}' for i in range(1, 8)])
    st.session_state.setdefault('PANSS_GENERAL_ITEMS', [f'panss_gen_{i}' for i in range(1, 17)])
    st.session_state.setdefault('CALGARY_ITEMS', [f'calgary_{i}' for i in range(1, 10)])
    st.session_state.setdefault('PSYRATS_AH_ITEMS', [f'psyrats_ah_{i}' for i in range(1, 12)])
    st.session_state.setdefault('PSYRATS_DEL_ITEMS', [f'psyrats_del_{i}' for i in range(1, 7)])

    # Define symptom categories for EMA data
    st.session_state.setdefault('POSITIVE_SYMPTOMS', ['hallucinations', 'delusions', 'paranoia', 'thought_disorder'])
    st.session_state.setdefault('NEGATIVE_SYMPTOMS', ['anhedonia', 'avolition', 'flat_affect', 'social_withdrawal'])
    st.session_state.setdefault('AFFECTIVE_SYMPTOMS', ['depression', 'anxiety', 'irritability', 'mood_swings'])
    ...
    # Combine symptom lists
    st.session_state['SYMPTOMS'] = (st.session_state.POSITIVE_SYMPTOMS + 
                               st.session_state.NEGATIVE_SYMPTOMS + 
                               st.session_state.AFFECTIVE_SYMPTOMS + 
    ...
    # Check patient selection requirement
        ...
        elif page_selected == "Analyse des Interventions Pharmacologiques": protocol_analysis_page()
